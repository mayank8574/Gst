...
try
{
    ...
    if(pid.trim().equals("3"))
    {
      ...
      query="delete from registration where id="+id.trim();
      ps=con.prepareStatement(query);
      ps.executeUpdate();
      pid="1";
    }
    else if(pid.trim().equals("4"))
    {
      
      query="update login set status='Blocked' where id="+id.trim();
      ps=con.prepareStatement(query);
      ps.executeUpdate();
      query="update user_detail set status='Blocked' where id="+id.trim();
      ps=con.prepareStatement(query);
      ps.executeUpdate();
      pid="1";
    }
    else if(pid.trim().equals("5"))
    {
      
      query="update login set status='Active' where id="+id.trim();
      ps=con.prepareStatement(query);
      ps.executeUpdate();
      query="update user_detail set status='Active' where id="+id.trim();
      ps=con.prepareStatement(query);
      ps.executeUpdate();
      pid="1";
    }
    ...
    {
      query="select * from user_detail where id="+id.trim();
      ps=con.prepareStatement(query);
      rs=ps.executeQuery();
      rs.next();
      ...
      session.setAttribute("gstno", rs.getString(13));
      session.setAttribute("status", rs.getString(14));
          ...
          <table class="table table-hover table-bordered" id="sampleTable">
            ...
            <%
                ...
                try
                {
                    ...
                    con = DriverManager.getConnection("jdbc:mysql://localhost:3306/gst?zeroDateTimeBehavior=convertToNull", "root", "");
                    query="select id,name,email_id,panno,aadharno from user_detail where id<>0 order by id desc";
                    ps=con.prepareStatement(query);
                    rs=ps.executeQuery();
                    int n1=0;
                    while(rs.next())
                    {
                        n1++;
                          ...
                          <tr>
                            ...
                            <td><div align="center">
                              <a href="client.jsp?id=<%=rs.getString(1)%>&pid=2"  style="text-decoration:none"><img src="Images/edit.png" alt="Display Record" title="Display Record" border="0" /></a>
                              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                              <a href="client.jsp?id=<%=rs.getString(1)%>&pid=3" style="text-decoration:none"><img src="Images/del.png" alt="Delete Record" title="Delete Record" border="0" /></a>
                             </div></td>
                          </tr>
                    <%
                    }
              ...
              <form method="post" onsubmit="return validate();" name="form" class="row">
                <div class="col-lg-6">
                  ...
                  <div class="form-group">
                    ...
                    <input class="form-control" id="exampleInputPassword1" name="rid" type="text" value="<%=(String)session.getAttribute("gstno")%>" readonly>
                  </div>
                  ...
                  <div class="tile-footer">
                    ...
                    <a href="client.jsp?id=<%=(String)session.getAttribute("id")%>&pid=4" class="btn btn-danger">Block</a>
                    <% } else { %>
                    <a href="client.jsp?id=<%=(String)session.getAttribute("id")%>&pid=5" class="btn btn-danger">Unblock</a>
                    <% } %>
                    <a href="client.jsp?id=<%=(String)session.getAttribute("id")%>&pid=3" class="btn btn-danger">Delete</a>
                    <a href="client.jsp?id=-1&pid=1" class="btn btn-warning">Back</a>
                  </div>
